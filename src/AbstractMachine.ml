...
type value = Int of int | Bool of bool | Closure of (instruction list)*environment
...
let apply_op op v1 v2 = match op,v1,v2 with
    | op,Int i,Int j -> Int (Interpreter.interpretIntBinOp op i j)
    | op,Bool i,Bool j -> Bool (Interpreter.interpretBoolBinOp op i j)
...
let step (s:state) = match s with
    | (Access n)::c,e,s,r -> c,e,[nth e n],r
    ...
    | (Op op)::c,e,v::w::s,r -> c,e,[apply_op op v w],r
